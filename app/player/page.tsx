...
import { Play, Pause, Rewind, FastForward, Volume2, Info, RotateCcw } from 'lucide-react';
...
export default function Player() {
  ...
  // Load song data with proper emotion analysis
  useEffect(() => {
    async function loadSong() {
      if (mode === 'upload') {
        ...
        if (url) {
          ...
          // Extract track info from URL (basic)
          const trackId = url.split('/track/')[1]?.split('?')[0];
          const trackTitle = `Spotify Track ${trackId?.substring(0, 8)}`;
  ...
  // Update current emotion for uploaded files
  useEffect(() => {
  ...
  const handleSpotifyEmotionChange = useCallback((emotion: any) => {
        ...
        <div className="relative z-10 p-4 pt-24 sm:p-6 sm:pt-24 flex flex-col items-center justify-center min-h-screen">
              ...
              <div className="mb-8 text-center">
                <div className="mb-4 flex gap-2 justify-center">
                  ...
                  >
                    ...
                    {spotifyIsPlaying ? 'Pause Sync' : 'Start Sync'}
                  </Button>
                  <Button 
                    onClick={resetSpotifySync}
                    variant="outline"
          ...
          {mode === "upload" ? (
            <div className="w-full max-w-md mb-8">
              <div className="glass-morphism-light rounded-3xl p-6 shadow-2xl">
                ...
                <div className="flex items-center justify-center gap-4 mb-4">
                  <Button variant="ghost" size="icon" className="text-gray-600 hover:text-black" onClick={() => handleSeek([Math.max(0, currentTime - 10)])}><Rewind className="w-5 h-5" /></Button>
                  <Button size="icon" className="w-14 h-14 bg-white/80 hover:bg-white text-gray-800 rounded-full shadow-lg" onClick={togglePlayPause}>
                    {isPlaying ? <Pause className="w-7 h-7" /> : <Play className="w-7 h-7" />}
                  </Button>
                  <Button variant="ghost" size="icon" className="text-gray-600 hover:text-black" onClick={() => handleSeek([Math.min(duration, currentTime + 10)])}><FastForward className="w-5 h-5" /></Button>
                </div>
            ...
            <div className="w-full max-w-md mb-8 glass-morphism-light rounded-3xl p-6 text-center">
               <h2 className="text-xl font-semibold text-gray-800 mb-1">Live Emotion Timeline</h2>
               <p className="text-gray-500 mb-4 flex items-center justify-center gap-2">
                  Emotions sync with song progression
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger>
                      ...
                      <TooltipContent>
                        <p>Emotions are analyzed and timed to match the song&apos;s lyrical content and mood changes.</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
               </p>
                      ...
                      <span 
                        ...
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 ${
                          currentEmotion?.lyric === e.lyric
                            ? 'bg-primary/80 text-white shadow-lg scale-110 ring-2 ring-primary/30' 
